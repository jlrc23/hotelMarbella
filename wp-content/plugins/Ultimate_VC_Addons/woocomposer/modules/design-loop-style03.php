<?php
function WooComposer_Loop_style03($atts,$element){
	...
	$sale_price = $shortcode = $on_sale_alignment = $on_sale_style = '';
	...
	$output .= '<div class="woocomposer" data-columns="'.$col.'">';
		$query = new WP_Query( $args );
		if($query->have_posts()):
			while ( $query->have_posts() ) : $query->the_post();
				$product_id = get_the_ID();
				$uid = uniqid();
				$output .= '<div id="product-'.$uid.'" class="'.$class.' wpb_column column_container wooproduct" data-animation="animated '.$product_animation.'">';
				if($element == 'carousel'){
				...
				if(in_array("quick",$elemets)){
					$output .= '<div class="wcmp-quick-view-wrapper" data-columns="'.$col.'">';
						...
						if(!empty($attachment_ids) && count($attachment_ids) > 1){
							...
							$output .= '<div class="wcmp-single-image wcmp-quickview-img"><img class="wcmp-img" src="'.$src.'" data-src="'.$product_img[0].'"/></div>';
						}
						if($quick_view_style == "popup"){
						...
						$output .= ob_get_clean();
						if($quick_view_style !== "popup"){
							$output .= '<div class="wcmp-close"><i class="wooicon-cross2"></i></div>';
						}
						$output .= '</div>';
	...
	if($pagination == "enable"){
		$output .= '<div class="woocomposer-paginate">';
