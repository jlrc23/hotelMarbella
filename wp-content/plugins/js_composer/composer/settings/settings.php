...
class WPBakeryVisualComposerSettings {
    ...
    public function __construct($composer) {
        ...
        if($composer->isPlugin() && !$composer->settingsAsTheme() && $this->showConfigurationTabs()) {
...
EOF;
        ...
        <form action="options.php" method="post" id="vc-settings-<?php echo $tab ?>" class="vc-settings-tab-content<?php echo ($this->active_tab == $tab ? ' vc-settings-tab-content-active' : '').$css ?>"<?php echo apply_filters('vc_setting-tab-form-'.$tab, '') ?>>
            ...
            <a href="#" class="button vc-activate-license-button" id="vc-settings-activate-license"><?php empty($license_activation_key) ? _e('Activate license', 'js_composer') : _e('Deactivate license', 'js_composer') ?></a> <span class="vc-updater-spinner-wrapper" style="display: none;" id="vc-updater-spinner"><img src="<?php echo get_site_url() ?>/wp-admin/images/wpspin_light.gif"  /></span>
            <?php endif; ?>
        </form>
        <?php endif; ?>
