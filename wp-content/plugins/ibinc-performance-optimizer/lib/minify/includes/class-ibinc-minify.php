...
class IBINC_MINIFY extends IBINC_FRAMEWORK {
	...
	{
		add_options_page(__('IBINC Minify'), 'IBINC Minify', IBINC_MINIFY_CAPABILITY, IBINC_MINIFY_OPTION_GENERAL, array($this, 'build_option_pages'));
	}
	...
	{
		if (!current_user_can(IBINC_MINIFY_CAPABILITY))
			wp_die(__('You do not have sufficient permissions to access this page.'));

		// Init the class
		$page = $_GET['page'];		
		$ibinc_option_page = new IBINC_OPTION_PAGE($page, $this->site_options);
		
		$options = array();
		
if (!empty($page))
{	
	if ($page == IBINC_MINIFY_OPTION_GENERAL)
	{
		$form = array(
			...
			'item_labels'	=> array
			(
				__('General Options', 'ibinc-minify'),
				__('Minify JS files automatically?', 'ibinc-minify'),
				__('Minify CSS files automatically?', 'ibinc-minify'),
				__('Minify <code>bloginfo()</code> stylesheets?', 'ibinc-minify'),
				__('Minifying Options', 'ibinc-minify'),
				__('Minify URL (double-click to edit)', 'ibinc-minify'),
				__('Cache directory (double-click to edit)', 'ibinc-minify'),
				__('One minify string will contain', 'ibinc-minify'),
				__('Append the minify string with', 'ibinc-minify'),
				__('Minifying Scripts Options', 'ibinc-minify'),
				__('Scripts to be minified in header', 'ibinc-minify'),
				__('Scripts to be minified in footer', 'ibinc-minify'),
				__('Scripts to be minified and then printed separately', 'ibinc-minify'),
				__('Scripts to be ignored (not minified)', 'ibinc-minify')
			),
			...
			'heading'			=> array(
				'h1'	=> '',
				'h2'	=> __('<em>Options that affect both your stylesheets and scripts.</em>', 'ibinc-minify'),
				'h3'	=> sprintf(__('<em>You can force the position of each script using those inputs below (e.g. you have a script registered in the header but you want to minify it in the footer instead). If you are still unsure, please read more <a href="%s#positioning-your-scripts">here</a>. Type in one script handle (<strong>NOT filename</strong>) per line.</em>', 'ibinc-minify'), $this->plugin_url)
			),
			'select' => array(
				'select_time_type' => array(
					__('second(s)', 'ibinc-minify') => 1,
					__('minute(s)', 'ibinc-minify') => 60,
					__('hour(s)', 'ibinc-minify') => 3600,
					__('day(s)', 'ibinc-minify') => 86400
				),
				'select_buster_type' => array(
					__('Do not append anything', 'ibinc-minify') => 'none',
					__('Cache folder&#8217;s last modified time', 'ibinc-minify') => 'mtime',
					__('Your WordPress&#8217;s current version', 'ibinc-minify') => 'wpver',
					__('Your theme&#8217;s current version', 'ibinc-minify') => 'tver',
					__('A custom number', 'ibinc-minify') => 'custom'
				)
			),
			'checkbox'	=> array(
				'cb1' => array(__('you can still use <code>ibinc_minify()</code> helper function if you disable this.', 'ibinc-minify') => 'enable_min_js'),
				'cb3' => array(__('you can still use <code>ibinc_minify()</code> helper function if you disable this.', 'ibinc-minify') => 'enable_min_css'),
				'cb2' => array(__('most themes (e.g. Twenty Ten) use <code>bloginfo()</code> to print the main stylesheet (i.e. <code>style.css</code>) and IBINC Minify will not be able to add it to the main minify string. If you want to minify <code>style.css</code> with the rest of your css files, you must enqueue it.', 'ibinc-minify') => 'enable_bloginfo')
			),
			'input'	=> array(
				'input_minurl' => array('size' => 91, 'disabled' => ' readonly="readonly"', 'label' => sprintf(__('This should be set automatically. If you think the URL is too long, please read <a href="%s#customization">here</a> to know how to properly modify this.', 'ibinc-minify'), $this->plugin_url)),
				'input_cache_dir' => array('size' => 91, 'disabled' => ' readonly="readonly"', 'label' => '<br />' . sprintf(__('<strong>Important</strong>: Changing cache directory is a two-step process, which is described in details <a href="%s#advanced_customization" target="_blank">here</a>. Please note that cache directory must be writable (i.e. CHMOD to 755 or 777).', 'ibinc-minify'), $this->plugin_url)),
				'input_maxfiles' => array('size' => 3, 'label' => __('file(s) at most.', 'ibinc-minify')),
				'input_maxage' => array('size' => 5, 'label' => __('&mdash;', 'ibinc-minify')),
				'input_custom_buster' => array('pre' => __('<em>&rarr; /min/?f=file.js&amp;ver=</em> ', 'ibinc-minify'), 'size' => 12, 'label' => '.', 'disabled' => ' disabled="disabled"')
			),
			'textarea' => array
			(
			...
			'container'	=> array(
				'select_buster_type' => __('<em><strong>Note:</strong> When you append one of the things above you are basically telling browsers to clear their cached version of your CSS and JS files, which is very useful when you change source files. Use this feature wisely :).</em>', 'ibinc-minify')
			),
			'inline_fields' => array(
			...
			'inline' => array(
				'input_cache_dir' => '<br /><br /><input type="submit" class="button-secondary action" name="flush_cache" value="' . __('Flush the cache', 'ibinc-minify') . '" />'
			)
		);

		// Get the default options
		$options = $ibinc_option_page->get_options(array('input_minurl', 'input_cache_dir', 'input_maxfiles', 'input_header', 'input_footer', 'input_direct', 'input_ignore', 'input_custom_buster', 'select_buster_type', 'enable_min_js', 'enable_min_css', 'enable_bloginfo'), $this->options);

		// Get option from the database
		$options = $ibinc_option_page->get_db_options($page, $options);
		...
		if (isset($_POST['flush_cache']) && !$this->is_normal_admin())
		{
			...
			if ($deleted = self::flush_cache($options['input_cache_dir']))
				$this->add_notice('<strong>' . __('Notice', 'ibinc-minify') . ':</strong> ' . sprintf(__("<strong>%d</strong> cached files have been deleted successfully!", 'ibinc-minify'), $deleted));
			else
				$this->add_notice('<strong>' . __('Notice', 'ibinc-minify') . ':</strong> ' . __("Could not delete any cached files. Please manually check the cache directory.", 'ibinc-minify'));
		}

		// Get option from user input
		if (isset($_POST['submit_' . $ibinc_option_page->get_form_name()]) && isset($options) && is_array($options))
		{
			check_admin_referer($page);
			foreach ($options as $key => &$option)
			{
				...
				{
					if (isset($_POST[$key]))
						$ibinc_option_page->format_field($key, $option_formats);
			...
			$this->add_notice(__('All options have been saved.', 'ibinc-minify'));
		}
		...
		// [WPMS Compatible]
		if ($this->is_normal_admin())
			$ibinc_option_page->kill_html_fields($form, array(4,5));
		// Cache buster system
		...
		if (!file_exists($options['input_cache_dir']) || !is_writable($options['input_cache_dir']))
			$this->add_notice('<strong>' . __('Warning') . ':</strong> ' . __("Cache directory does not exist or is not writable. Please try CHMOD your cache directory to 755. If you still see this warning, CHMOD to 777.", 'ibinc-minify'));

?>
	<script type="text/javascript">
		jQuery(document).ready(function(){
			jQuery('.ibinc-option-page :input[readonly]').dblclick(function(){
...
<?php
		...
		$ibinc_option_page->init($form, $options, $this->form_tabs);

		// Build the option page	
		echo $ibinc_option_page->generate_html_form();
	}
