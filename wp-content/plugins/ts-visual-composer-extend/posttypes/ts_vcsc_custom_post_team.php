<?php
	...
	function TS_VCSC_Team_Meta_Boxes($meta_boxes) {
		...
		$meta_boxes['ts_vcsc_team_basic'] = array(
			'id'                		=> 'ts_vcsc_team_basic',            // meta box id, unique per meta box 
			...
			'fields' 					=> array(
				...
				array('name' => '<i class="ts-teamicon-email2 ts-font-icon"></i> Email Address:', 'std' => '', 'desc' => '', 'id' => $prefixB . 'email', 'type' => 'text_email'),
				...
				array('name' => 'Label for Portfolio URL:', 'std' => '', 'desc' => 'If left empty, the actual URL to the personal site will be shown.', 'id' => $prefixB . 'otherlabel', 'type' => 'text_medium'),
		...
		$meta_boxes['ts_vcsc_team_file'] = array(
			...
			'fields' 					=> array(
				...
				array('name' => 'Button Label:', 'std' => 'View Teammate', 'desc' => '', 'id' => $prefixA . 'buttonlabel', 'type' => 'text_medium'),
	...
	function TS_VCSC_Team_Post_Files() {
		...
		if ($typenow == 'ts_team') {
			if ($pagenow == 'post-new.php' || $pagenow=='post.php') {
				...
				wp_enqueue_style('ts-extend-uitotop', 							TS_VCSC_GetResourceURL('css/jquery.ui.totop.css'), null, false, 'all');
				wp_enqueue_script('ts-extend-uitotop', 							TS_VCSC_GetResourceURL('js/jquery.ui.totop.min.js'), array('jquery'), '3.8', true);
				wp_enqueue_script('jquery-easing', 								TS_VCSC_GetResourceURL('js/jquery.easing.js'), array('jquery'), '3.8', true);
				wp_enqueue_style('ts-font-teammates',							TS_VCSC_GetResourceURL('css/ts-font-teammates.css'), null, false, 'all');
				wp_enqueue_style('ts-extend-select2',							TS_VCSC_GetResourceURL('css/jquery.select2.css'), null, false, 'all');
				wp_enqueue_script('ts-extend-select2',							TS_VCSC_GetResourceURL('js/jquery.select2.min.js'), array('jquery'), false, true);
				wp_enqueue_script('ts-extend-teammates', 						TS_VCSC_GetResourceURL('js/ts-visual-composer-extend-team.min.js'), array('jquery',), false, true);
			}
		}
	}
	add_action('admin_init',									'TS_VCSC_Team_Post_Files');
