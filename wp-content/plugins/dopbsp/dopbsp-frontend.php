    ...
    if (!class_exists("DOPBookingSystemPROFrontEnd")){
        class DOPBookingSystemPROFrontEnd{
            ...
            function calendarShortcode($atts){// Read Shortcodes.
                ...
                if ($atts['woocommerce'] == 'true'){
                    array_push($data, '         jQuery(".single_add_to_cart_button").before("<input type=\"hidden\" name=\"add-to-cart\" value=\"'.$post->ID.'\"/>");');
                    array_push($data, '         jQuery(".DOPBookingSystemPROContainer").before("<input type=\"hidden\" name=\"add-to-cart\" value=\"'.$post->ID.'\"/>");');
                }
