...
* Description             : Booking System PRO Forms Class.
*/
            ...
            function listForms(){
                ...
                if (!is_super_admin()){
                    if (wp_get_current_user()->roles[0] == 'administrator' && $this->administratorHasPermissions(wp_get_current_user()->ID)){
                        $booking_forms = $wpdb->get_results('SELECT * FROM '.DOPBSP_Forms_table.' ORDER BY id ASC');
                    }
                    else{
                        $booking_forms = $wpdb->get_results('SELECT * FROM '.DOPBSP_Forms_table.' WHERE user_id="'.wp_get_current_user()->ID.'" OR user_id="0" ORDER BY id ASC');
                    }
                } else {
                    $booking_forms = $wpdb->get_results('SELECT * FROM '.DOPBSP_Forms_table.' ORDER BY id ASC');
                }
...
// Booking Forms            
            function showBookingForms(){// Show Forms List.
                ...
                if (wp_get_current_user()->roles[0] == 'administrator' && $this->administratorHasPermissions(wp_get_current_user()->ID)){
                    $booking_forms = $wpdb->get_results('SELECT * FROM '.DOPBSP_Forms_table.' ORDER BY id DESC');
                }
                else{
                    $booking_forms = $wpdb->get_results('SELECT * FROM '.DOPBSP_Forms_table.' WHERE user_id="'.wp_get_current_user()->ID.'" OR user_id="0" ORDER BY id DESC');
                }
                
                if ($wpdb->num_rows != 0){
                    if ($booking_forms){
                        foreach ($booking_forms as $booking_form){
                            array_push($booking_forms_HTML, '<li class="item" id="DOPBSP-ID-'.$booking_form->id.'"><span class="id">ID '.$booking_form->id.':</span> <span class="name">'.$this->shortName($booking_form->name, 25).'</span></li>');  
                        }
                    }
                }
                else{
                    array_push($booking_forms_HTML, '<li class="no-data">'.DOPBSP_NO_BOOKING_FORMS.'</li>');
                }
                array_push($booking_forms_HTML, '</ul>');
            ...
            function addBookingForm(){// Add Booking form.
                global $wpdb;
                                
                $wpdb->insert(DOPBSP_Forms_table, array('user_id' => wp_get_current_user()->ID,
                                                        'name' => DOPBSP_ADD_BOOKING_FORM_NAME));                            
                $this->showBookingForms();
                echo '';
            ...
            function showBookingForm(){// Show Calendar.
                if (isset($_POST['id'])){
                    ...
                    $data = array();
                    $form = $wpdb->get_row('SELECT * FROM '.DOPBSP_Forms_table.' WHERE id="'.$_POST['id'].'"');
            ...
            function deleteBookingForm(){// Delete Booking Form.
                global $wpdb;

                $wpdb->query('DELETE FROM '.DOPBSP_Forms_table.' WHERE id="'.$_POST['id'].'"');
                $fields = $wpdb->get_results('SELECT * FROM '.DOPBSP_Forms_Fields_table.' WHERE form_id='.$_POST['id']);
                
                foreach($fields as $field){
                    $wpdb->query('DELETE FROM '.DOPBSP_Forms_Select_Options_table.' WHERE field_id="'.$field->id.'"');
                }
                $wpdb->query('DELETE FROM '.DOPBSP_Forms_Fields_table.' WHERE form_id="'.$_POST['id'].'"');
                
                $forms = $wpdb->get_results('SELECT * FROM '.DOPBSP_Calendars_table.' ORDER BY id DESC');
                echo $wpdb->num_rows;
            ...
            function showBookingFormFields(){
                ...
                $fields = $wpdb->get_results('SELECT * FROM '.DOPBSP_Forms_Fields_table.' WHERE form_id='.$_POST['booking_form_id'].' ORDER BY position ASC');
                ...
                if ($wpdb->num_rows > 0){
                    ...
                    foreach($fields as $field){
                        ...
                        echo '  <span class="booking-form-field-name-wrapper">'.
                                    ...
                                    <a href="javascript:dopbspExpandBookingFormField('.$field->id.')" class="booking-form-field-show-settings-button">'.DOPBSP_BOOKING_FORM_FIELDS_SHOW_SETTINGS.'</a>   
                        ...
                        echo '      <div class="settings-box">
                                        <label for="DOPBSP-admin-form-field-language-'.$field->id.'">'.DOPBSP_BOOKING_FORM_FIELDS_LANGUAGE_LABEL.'</label>';
                                        echo $this->bookingFormLanguages($field->id);
                        echo '          <a href="javascript:void()" class="help"><span>'.DOPBSP_BOOKING_FORM_FIELDS_LANGUAGE_INFO.'</span></a>
                                        <br class="DOPBSP-clear" />
                                    </div>';
                        // Name
                        echo '      <div class="settings-box">
                        ...
                        if ($field->type == 'select'){
                            echo '  <div class="settings-box">
                                        <label>'.DOPBSP_BOOKING_FORM_FIELDS_SELECT_OPTIONS_LABEL.'</label>
                                        <div class="booking-form-field-select-options" id="booking-form-field-select-options-'.$field->id.'">
                                            <a class="add" href="javascript:dopbspBookingFormFieldSelectAddOption('.$field->id.')" title="'.DOPBSP_BOOKING_FORM_FIELDS_SELECT_ADD_OPTION.'"></a>
                                            <a href="javascript:void()" class="help"><span>'.DOPBSP_BOOKING_FORM_FIELDS_SELECT_OPTIONS_INFO.'</span></a>
                                            <span class="loader" id="booking-form-loader-field-select-'.$field->id.'"></span>
                                            <br class="DOPBSP-clear" /><br />';
                            ...
                            foreach ($options as $option){
                                ...
                                echo '      <div class="option-box" id="booking-form-field-select-option-'.$option->id.'">
                                                ...
                                                <input type="hidden" class="dopbsp-booking-form-options-curent-language" id="booking-form-field-select-option-language-'.$option->id.'" name="language" value="'.$current_backend_language.'"/>
                                                <br class="DOPBSP-clear">
                                            </div>';
                            }
                            echo '      </div>
...
<?php
            ...
            function addBookingFormField(){
                ...
                foreach ($positionsp as $position){
                    if ($position){
                        ...
                        $wpdb->update(DOPBSP_Forms_Fields_table, $data, array('id' => $pos_id));
                    }
            ...
            function deleteBookingFormField(){
                ...
                $wpdb->query('DELETE FROM '.DOPBSP_Forms_Fields_table.' WHERE id="'.$_POST['fieldId'].'"');
                $wpdb->query('DELETE FROM '.DOPBSP_Forms_Select_Options_table.' WHERE field_id="'.$_POST['fieldId'].'"');
            ...
            function addBookingFormFieldSelectOption(){
                ...
                mysql_query('BEGIN');
                $wpdb->insert(DOPBSP_Forms_Select_Options_table, array('field_id' => $_POST['field_id'],
            ...
            function deleteBookingFormFieldSelectOption(){
                ...
                $wpdb->query('DELETE FROM '.DOPBSP_Forms_Select_Options_table.' WHERE id="'.$_POST['id'].'"');
                echo $_POST['id'];
