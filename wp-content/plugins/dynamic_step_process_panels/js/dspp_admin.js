jQuery(document).ready(function($){
    ...
    $("#stepscount").bind("keyup paste change", function(){
		...
		for(var i=0; i<number; i++)
		{
			...
			if($("#dspp_table_content"+(i+1)).length==0)
			{
				...
				html += "	<tr valign='top'>";
				html += "		<th scope='row'>";
				html += "			<label for='width" + (i+1) + "''>Width</label>";
				html += "		</th>";
				html += "		<td>";
				html += "			<input type='text' class='regular-text' value='' id='width" + (i+1) + "'' name='step_width[]'>";
				html += "			<span class='description'>Defines the width of the step (in px).</span>";
				html += "		</td>";
				html += "	</tr>";
				html += "	<tr valign='top'>";
				html += "		<th scope='row'>";
				...
				html += "		</th>";
				html += "		<td>";
				html += "			<input type='text' class='regular-text' value='div' id='contenttag" + (i+1) + "'' name='step_content_tag[]'>";
				html += "			<span class='description'>Defines the html tag of the step content node.</span>";
				html += "		</td>";
				html += "	</tr>";
				html += "	<tr valign='top'>";
				html += "		<th scope='row'>";
				html += "			<label for='contentid" + (i+1) + "''>Content id</label>";
				html += "		</th>";
				html += "		<td>";
				html += "			<input type='text' class='regular-text' value='content" + (i+1) + "_" + $("#shortcodeId").val() + "'' id='contentid" + (i+1) + "'' name='step_content_id[]'>";
				...
				html += "		</th>";
				html += "		<td>";
				html += "			<input type='text' class='regular-text' value='' id='contentclass" + (i+1) + "'' name='step_content_class[]'>";
			...
			$.ajax({
					...
					success: function(json){
						...
						$.each(json, function(key, val){
							if(key=="title" || key=="width" || key=="tag" || key=="href" || key=="class" || key=="label" || key=="content" || key=="contentfromfile" || key=="contenttag" || key=="contentid" || key=="contentclass")
							{
	...
	if(config.selectedShortcodeId!="")
		$("#editShortcodeId").val("dspp_shortcode_settings_" + config.selectedShortcodeId).trigger("change");
