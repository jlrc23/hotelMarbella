...
if(!class_exists("WooComposer")){
	class WooComposer{
		...
		{
			$output = $asc = $desc = $post_count = $shortcode_str = '';
			...
			$output .= wp_dropdown_categories( array('taxonomy'=>'product_cat','echo' => false,)).'</div>';
			$output .= '<!-- '.$value.' -->';
...
/*
		...
		function woo_product_categories($settings, $value){
			...
			$output .= '<script type="text/javascript">
							//jQuery("select[name=\'cat\']").attr({"multiple":"true","style":"display:block","class":"sel_cat","id":"sel2_cat"});
