...
function fblb_instagram_get_latest() {
    $options = fblb_get_options();
    $images = array();
    if($options['INST_access_token'] != null)
    {
        if(isset($options['INST_hashtag']) && trim($options['INST_hashtag']) != "" && preg_match("/[a-zA-Z0-9_\-]+/i", $options['INST_hashtag']))
        {
            $hashtag = $options['INST_hashtag'];
            if (substr($hashtag, 0, 1) == '#')
            {
                $hashtag = substr($hashtag, 1);
			}
            
            $apiurl = "https://api.instagram.com/v1/tags/".$hashtag."/media/recent?count=".$options['INST_NumberofPosts']."&access_token=".$options['INST_access_token'];
        }
        else
        {
            $apiurl = "https://api.instagram.com/v1/users/self/media/recent?count=".$options['INST_NumberofPosts']."&access_token=".$options['INST_access_token'];
        }
        ...
        if(!is_wp_error($response) && $response['response']['code'] < 400 && $response['response']['code'] >= 200)
        {
            ...
            if($data->meta->code == 200)
            {
                foreach($data->data as $item) {
                    if (isset($options['INST_hashtag'], $item->caption->text))
                    {
                        $image_title = $item->user->username.': &quot;'.filter_var($item->caption->text, FILTER_SANITIZE_STRING).'&quot;';
                    }
                    elseif(isset($options['INST_hashtag']) && !isset($item->caption->text))
                    {
