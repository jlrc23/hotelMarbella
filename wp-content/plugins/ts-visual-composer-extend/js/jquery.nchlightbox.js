...
( function( $, ndef ){
	...
	ui = {
		...
		slideOut : function( g, fn ){
			...
			eff( el, stage, true );
			c.stop().animate({ scrollLeft : left( - stage * _.wsize ) },{ duration: 500, complete: function(){
		...
		slideIn : function( g, fn ){
			...
			m.blocks.media.css( 'visibility', 'visible' );
			c.stop().scrollLeft( left( - stage * _.wsize ) ).animate({ scrollLeft : left( 0 ) }, 500, function(){ c.scrollLeft( left( 0 ) ) } );
	...
	types = {
		youtube : {
			...
			create : function( e ){
				// Custom Addition
				if (e.opts.play == "true") {
					return '<iframe class="nch-lb-iframe nch-video-youtube" src="http://www.youtube.com/embed/' + e.opts.videoid + '?autoplay=1&wmode=opaque" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowfullscreen></iframe>';
				} else {
					return '<iframe class="nch-lb-iframe nch-video-youtube" src="http://www.youtube.com/embed/' + e.opts.videoid + '?autoplay=0&mode=opaque" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowfullscreen></iframe>';
				}
				// Custom Addition
			},
			thumb : function( e, fn ){
				utils.loadThumb.call( e, e.opts.src = 'http://img.youtube.com/vi/' + e.opts.videoid + '/hqdefault.jpg', fn );
			}			
		},
		...
		vimeo : {
			...
			create : function( e ){
				// Custom Addition
				if (e.opts.play == "true") {
					return '<iframe class="nch-lb-iframe nch-video-vimeo" src="http://player.vimeo.com/video/' + e.opts.videoid + '?autoplay=1" width="500" height="500" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
				} else {
					return '<iframe class="nch-lb-iframe nch-video-vimeo" src="http://player.vimeo.com/video/' + e.opts.videoid + '?autoplay=0" width="500" height="500" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
				}
				// Custom Addition
			},
			thumb : function( e, fn ){
				$.getJSON( 'http://vimeo.com/api/v2/video/' + e.opts.videoid + '.json?callback=?', { format: 'json' }, function( r ){ utils.loadThumb.call( e, e.opts.src = r[ 0 ][ 'thumbnail_large' ], fn ) } );
			}
		},
		
		// Custom Addition DailyMotion
		dailymotion : {
			is : function( e ){
				console.log(e);
			...
			create : function( e ){
				// Custom Addition
				if (e.opts.play == "true") {
					return '<iframe class="nch-lb-iframe nch-video-dailymotion" src="http://www.dailymotion.com/embed/video/' + e.opts.videoid + '?autoplay=1" width="500" height="500" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
				} else {
					return '<iframe class="nch-lb-iframe nch-video-dailymotion" src="http://www.dailymotion.com/embed/video/' + e.opts.videoid + '?autoplay=0" width="500" height="500" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
				}
			},
			thumb : function( e, fn ){
				utils.loadThumb.call( e, e.opts.src = 'http://www.dailymotion.com/thumbnail/video/' + e.opts.videoid + '', fn );
			}
		},
	...
	methods = {
		...
		open: function( name ){
			...
			if( k ){
				...
				window.location.hash = '#' + defaults.hashpath + k.opts.hash;
		...
		next : function( e ){
			...
			window.location.hash = '#' + defaults.hashpath + t.opts.hash;
	...
	utils = {
		...
		useTouch : function( el ){
			...
			el.hammer({ prevent_default: true }).on( 'dragstart', function( e ){
				 ...
				 */
				if( isTouchDevice === ndef && ( isTouchDevice = !( e.gesture.srcEvent.changedTouches === ndef ) ) ) _.b.addClass( 'nch-lb-photo-menu-visible' );
				
				if( _.zooming || ! _.current || e.gesture === ndef ) return true;
				
				var t = $( e.target ), q = e.timeStamp, x, o = t, p, c, m, d, r = false, a, g = _.wsize / 3, n = _.current, dr = 'nch-lb-dragging', u = true, tr = n.css3, eff = utils.getFX( n.set[ n.current ].opts.effect, n.set[ n.current ] ), alter = ( eff( 0, 0, 0, 1 ) !== ndef ), left = function( w ){ return ( alter ? eff( 0, 0, 0, w ) : w ) + _.wsize };
		...
		getMediaProperties : function( i ){
			...
			// Custom Addition DailyMotion
			r.url = ( t == 'youtube' || t == 'vimeo' || t == 'dailymotion' || t == 'iframe' ) ? r.media : window.location.href;
		...
		renderMedia : function( m ){
			...
			if( ! m.target ) {
				...
				// Custom Addition DailyMotion
				if( t == 'youtube' || t == 'vimeo' || t == 'dailymotion' ) {
		...
		updateMediaLayout : function( m ){
			 ...
			 */
			// Custom Addition DailyMotion
			if( t == 'youtube' || t == 'vimeo' || t == 'dailymotion' ) os = m.nsize = m.group.opts.size.split( ',' );
			 ...
			 */
			// Custom Addition DailyMotion
			if( t == 'image' || t == 'youtube' || t == 'vimeo' || t == 'dailymotion' ){
			...
			// Custom Addition DailyMotion
			if( t == 'iframe' || t == 'youtube' || t == 'vimeo' || t == 'dailymotion' ){
		...
		toMedia : function( i ){
			...
			utils.getMediaType( i, function( t ){
				...
				var fn = function() { gotMediaType( t ) };
		...
		renderLayout : function( g ){
			 ...
			 * Create controls
			 */
			var st, ct = bl.controls = $( '<ul class="nch-lb-controls"></ul>' ).appendTo( bt ), cti = { prev: 'left', play: 'play', next: 'right', title: false, exit: 'exit', fs: 'fs', info: 'info' };
			...
			for( var i in cti ) {
				ct.append( st = '<li class="nch-lb-' + i + '">' + ( cti[ i ] ? '<div class="nch-lb-icon-' + cti[ i ] + '"></div>' : '' ) + '</li>' );
			 ...
			 * Add photo options
			 */
			var po = bl.menu = $( '<ul class="nch-lb-photo-menu"></ul>' ).appendTo( md ), pop = [ 'zoom', 'share' ];
			for( var i = 0; i < pop.length; i++ ) po.append( '<li class="nch-lb-' + pop[ i ] + '"><div class="nch-lb-icon-' + pop[ i ] + '"></li>' );
			
			/**
			 * Add info panel
			 */
			var np = bl.info = $( '<div class="nch-lb-project"><div class="nch-lb-info-panel"><div class="nch-lb-icon-close"></div></div></div>' ).appendTo( m );
			...
			$( 'div.nch-lb-icon-close', m ).each( function(){ $( this ).click( utils.toggleInfo ) } );
		...
		fs : {
			...
			toggle: function(){
				...
				var c = 'nch-lb-onfs', b = _.current.main;
				
				if( utils.fs.on ){ b.removeClass( c ); utils.fs.exit() }
				else { b.addClass( c ); utils.fs.request( document.documentElement ) };
		...
		hashChange : function(){
			...
			window.setInterval( function(){
				if( h != window.location.hash && ( h = window.location.hash ).search( '#' + defaults.hashpath ) == 0 ) {
		...
		getMediaType : function( e, fn, i ){
			...
			if( o.type != false ){
				...
				if( ! e.target )
					// Custom Addition DailyMotion
					if( o.type == 'youtube' || o.type == 'vimeo' || o.type == 'dailymotion' )						
						if( o.videoid === ndef ) 
							types[ o.type ].is( e );
		...
		getThumbnail : function( e, fn ){
			...
			// Custom Addition DailyMotion
			} else if( t == 'youtube' || t == 'vimeo' || t == 'dailymotion' ){
...
(function( $, ndef ){
	 ...
	 * Property shortcuts
	 */
	var p = 'perspective', sx='skewX', sy = 'skewY', r = 'rotate', rx = 'rotateX', ry = 'rotateY', tx = 'translateX', ty = 'translateY', tz = 'translateZ', o = 'opacity', md = '50% 50%', dl = 400, s = 'scale';	
...
( function( $ ){
	...
	var api = { 
		...
		fb: {
			...
			create: '<iframe class="fb-like" src="//www.facebook.com/plugins/like.php?layout=standard&amp;action=like&amp;show_faces=false&amp;share=false&amp;height=20&amp;href={url}" scrolling="no" frameborder="0" style="border:none; overflow:hidden; height:20px;" allowTransparency="true"></iframe>'
		},
		...
		loadTiles: function( fn ){
			...
			function setWidth( index, width, height ){
				...
				// Start Custom Addition
					...
					d.append( '<div class="nchgrid-caption"></div>' ).click( utils.triggerClick );
				}
				// End Custom Addition
