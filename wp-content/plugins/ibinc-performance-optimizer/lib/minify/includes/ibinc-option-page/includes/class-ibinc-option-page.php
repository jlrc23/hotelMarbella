...
class IBINC_OPTION_PAGE {
	 ...
	 * Constructor
	 */
	function __construct($form_name = 'ibinc_option_page', $site_options = array(), $domain = '')
	{
	...
	{		
		...
		if (sizeof($this->form_tabs) == 0)
			$this->form_tabs		= array(__('Plugin Configurations', 'ibinc-option-page'));		
	}
	...
	function generate_html_fields($type, $name)
	{
		...
		$input_class = ($type == 'heading') ? 'ibinc-option-page-heading-desc' : 'ibinc-option-page-inputs';
		...
		switch ($type)
		{
			case 'section':	
				...
				$item_label = '<span class="ibinc-opton-page-label">' . $this->form_item_labels[$item_key[0]] . $inline . '</span>';
			...
			default:
				...
				/*$item_label = (empty($this->form[$type][$name]['label'])) ? '<label class="ibinc-opton-page-label" for="' . $name . '">' . $this->form_item_labels[$item_key[0]] . '</label>' : '<span class="ibinc-opton-page-label">' . $this->form_item_labels[$item_key[0]] . '</span>';*/
				$item_label = ($type != 'checkbox' && $type != 'radio') ? '<label class="ibinc-opton-page-label" for="' . $name . '">' . $this->form_item_labels[$item_key[0]] . $inline . '</label>' : '<span class="ibinc-opton-page-label type-' . $type . '">' . $this->form_item_labels[$item_key[0]] . $inline . '</span>';
	...
	function generate_html_form()
	{
		...
		if (sizeof($this->form_tabs) >= 2)
			$return_str .= apply_filters('ibinc-admin-form-icon', '<div class="icon32" id="icon-options-general"><br></div>'  . "\n");
		else
		...
		{
			$count = 0;
			$return_str .= '<h2 class="ibinc-option-page-tabs">' . "\n";
			$return_str .= apply_filters('ibinc-admin-plugin-version', '') . "\n";
		...
		$return_str .= apply_filters('ibinc_option_before_form', '');
		echo $return_str;
		do_action('ibinc_option_action_before_form');
		$return_str = '';
		$return_str .= '<form class="ibinc-option-page" name="' . $this->form_name . '" method="post" action="">'  . "\n";
		...
		$return_str .= '</ul>' . "\n";		
		$return_str .= apply_filters('ibinc_option_before_submit_button', '');		
		echo $return_str;		
		do_action('ibinc_option_action_before_submit_button');		
		$return_str = '';
		$return_str .= apply_filters('ibinc_option_submit_button', '<p class="submit"><input type="submit" class="button-primary" name="submit_' . $this->form_name . '" value="' . __('Save Changes') . '" /></p>') . "\n";
		$return_str .= '</form>' . "\n";		
		$return_str .= '</div>' . "\n";
